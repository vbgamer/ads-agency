...
function getCorsHeaders(origin: string | null) {
  // Default origin = SITE_URL if configured, else "*" (server-to-server postbacks
  // don't send Origin headers, so we allow them through; browser requests
  // are restricted to ALLOWED_ORIGINS by exact match).
  const fallback = ALLOWED_ORIGINS[0] || "*"
  const allowed = origin && ALLOWED_ORIGINS.includes(origin) ? origin : fallback
  return {
